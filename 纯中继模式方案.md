...
interface ServerConfig {
  ...
  // 认证配置
  adminToken?: string;           // 管理页面访问 token
  requireClientToken?: boolean;  // 是否要求客户端 token 认证（默认: false）
}

// Token 管理
interface TokenEntry {
  token: string;                 // token 值
  clientId?: string;             // 绑定的客户端 ID（可选）
...
interface ClientConfig {
  ...
  // 认证 token
  token?: string;              // 可选，服务器启用了 token 认证时需要
...
# 指定端口和管理 token
cts start --port 9000 --admin-token my-secure-token
...
# 指定服务器和 token
cts start --server ws://server-ip:8200 --token my-token
...
```bash
# 添加映射：本地 8080 → clientB 的 3000
cts add 8080:3000:clientB

# 添加多个映射
cts add 8080:3000:clientB
cts add 22:22:home-pc
cts add 3306:3306:db-server

# 查看所有映射
cts list

# 移除指定映射
cts remove 8080:3000:clientB
...
└────────────────┘  中继   └────────────────┘  中继   └────────────────┘
     cts start                         cts start                  cts start
     cts add 8080:8080:computerB       --admin-token xxx          --token xxx
```
...
# 启动时设置管理 token
cts start --admin-token my-admin-token
...
# 指定服务器地址
cts start --server ws://server-ip:8200
...
# 添加端口映射
cts add 8080:3000:clientB
# 说明：本地 8080 端口映射到 clientB 的 3000 端口
...
```
格式: [localPort]:[remotePort]:[targetClientId]

示例:
  8080:3000:clientB    本地 8080 → clientB 的 3000
  22:22:home-pc       本地 22 → home-pc 的 22 (SSH)
  3306:3306:db-server 本地 3306 → db-server 的 3306 (MySQL)

移除命令:
  cts remove 8080:3000:clientB  移除指定映射
  cts remove 8080              移除本地 8080 的所有映射
```
...
**认证方式**: 需要使用管理 token 登录（启动时通过 `--admin-token` 设置）
...
- 流量统计
- 系统状态
- **Token 管理**：添加/删除客户端连接 token
- **配置导入/导出**：支持 JSON 格式配置文件
...
用途：
客户端连接时需要使用有效 token：
  cts start --server ws://server-ip:8200 --token your-token
```
...
功能：
- 查看当前所有映射
- 添加新映射（localPort:remotePort:targetClientId）
- 删除指定映射
- 批量导入映射（JSON 格式）
- 批量导出映射（JSON 格式）
- 映射状态实时更新（在线/离线）
...
导出格式 (JSON)：
  {
    "serverUrl": "ws://server-ip:8200",
    "token": "my-token",
    "autoReconnect": true,
    "proxies": [
      {"localPort": 8080, "remotePort": 3000, "targetClientId": "clientB"},
      {"localPort": 22, "remotePort": 22, "targetClientId": "home-pc"}
    ]
  }
